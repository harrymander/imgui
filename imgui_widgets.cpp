    ...
    if (render_cursor || render_selection)
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            if (col & IM_COL32_A_MASK)
                g.Font->RenderText(draw_window->DrawList, g.FontSize, draw_pos - draw_scroll, col, clip_rect.AsVec4(), ImStrv(buf_display, buf_display_end), wrap_width, ImDrawTextFlags_WrapKeepBlanks);
            //draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, col, ImStrv(buf_display, buf_display_end), wrap_width, is_multiline ? NULL : &clip_rect.AsVec4());
        }
